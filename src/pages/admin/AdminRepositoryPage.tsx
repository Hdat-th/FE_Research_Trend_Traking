...
import {
  repositoryAnomalies as seedAnomalies,
  repositoryCategories as seedCategories,
  repositoryPapers,
...
const AdminRepositoryPage = () => {
  const [activeTab, setActiveTab] = useState<'papers' | 'ontology'>('ontology');
  const [categories, setCategories] = useState<RepositoryCategory[]>(seedCategories);
  const [anomalies, setAnomalies] = useState<RepositoryAnomaly[]>(seedAnomalies);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        <AdminMetricCard label="Total Articles Verified" value={`${papers.filter((paper) => paper.status === 'VERIFIED').length}`} helper="Verified demo metadata records" icon="▣" accent="slate" />
        ...
        <AdminMetricCard label="Indexed Today" value={`+${papers.length}`} helper="Local mock ingestion count" icon="⟳" accent="slate" />
      </div>
