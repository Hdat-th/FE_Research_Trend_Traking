    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={() => runSync('OpenAlex Ingest')} disabled={isSyncing} className="rounded-md bg-[#4338ca] hover:bg-[#3730a3] px-4 py-2 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-60">{isSyncing ? 'Syncing...' : '↥ Ingest Now'}</button>
        </div>
      </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[1.25fr_0.85fr]">
        ...
        <AdminSectionCard title="API Authentication">
          <div className="space-y-4 p-5">
            ...
            <button onClick={saveKey} className="w-full rounded-md bg-[#4338ca] hover:bg-[#3730a3] px-4 py-2 text-sm font-bold text-white">Save Key</button>
          </div>
        </AdminSectionCard>
      </div>
      ...
      >
        <p className="text-sm text-slate-600">The exported file contains engine name, timestamp and current sync status. This simulates the api_sync_logs table used by the backend.</p>
      </AdminModal>
    </div>
  );
};
